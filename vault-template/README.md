...
00 Meta/
  ...
  Template/                Obsidian note templates (Source / Idea / Concept).
10 Notes/   20 Ideas/   30 Concept/   40 Write/   Inbox/
                           The four pipeline stages + quick capture (empty, with hints).
```
...
5. **Start ingesting:** open Claude Code in the vault and paste a URL, drop a file in `Inbox/`, or say "extract Atomic Habits." Claude runs the pipeline: source → atomic ideas → concepts. Everything lands as `review_status: unreviewed`.
